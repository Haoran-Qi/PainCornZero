...
def privateKeyToWif(private_key):
    return utils.base58CheckEncode(hex(0x80), private_key)

def wifToPrivateKey(wif):
    return utils.base58CheckDecode(wif)
...
def publicKeyToAddr(s):
    ...
    return utils.base58CheckEncode(hex(0), binascii.hexlify(ripemd160.digest()).decode('utf-8'))
